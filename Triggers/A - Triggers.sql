...
For individual INSERT/UPDATE/DELETE statements, SQL server manages its own transaction for that change in the db table; this is an implicit transaction.

SQL server does this two temporary tables it constructs for the DML statement. These temporary tables have special names:
-deleted
-inserted

From the perspective of an insert - we are adding a new row(s) of data to a table.
From the perspective of adelete - we are removing row(s) of data from a table.
...
Triggers are never called directly by our scripts, instead they are called  (or "triggered") by SQL server
    as it performs its internale transaction.
Triggers are "attached" to individual tables. If you drop the table, then the table is dropped too.

Triggers are used for:

-Performing complex validations that cannot be done with ordinary CHECK constraints or other constraints.
...
-An opportunity to do some archiving or auditing actions
-To prevent an INSERT, UPDATE or DELETE from happening (by doing  a ROLLBACK TRANSACTION)
*/
...
GO
-- The trigger we want to create here is bec we need to do some complex validation that cant be done in a regular CHECK constraint
...
--SELECT * FROM Student WHERE StudentID = 200312345 -- to check whi=o is this student
...
--How can we tell if  a course is being increased by more than 20%? We have to do a comparison between the "before" cost and the  "after" cost
...
UPDATE Course SET CourseCost = CourseCost * 1.195 -- An increased of 19.5%
...
-- Let's modify this record: 199899200 DMIT254 2005M 0.00 Y G
-- Test with good data
UPDATE  Registration
SET     Mark = 65,
        WithdrawYN = 'N'
